...
export const validateAd = (ad: Ad): ValidationErrors => {
  ...
  // Validate image URL format if provided (required for Facebook sync)
  if (ad.image_url && ad.image_url.trim().length > 0) {
    try {
      const imageUrl = new URL(ad.image_url);

      // Facebook requires HTTPS
      if (imageUrl.protocol !== 'https:') {
        errors.image_url = 'Image URL must use HTTPS (required by Facebook)';
      } else {
        // Validate image file extension
        const path = imageUrl.pathname.toLowerCase();
        const validExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp'];
        const hasValidExtension = validExtensions.some(
          ext => path.indexOf(ext, path.length - ext.length) !== -1
        );

        if (!hasValidExtension) {
          errors.image_url = 'Image URL must end with .jpg, .jpeg, .png, .gif, or .webp';
        }
      }
    } catch {
